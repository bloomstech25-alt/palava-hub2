...
- **Type**: Static web (Express static server)
- **Preview path**: `/web/`
- **Port**: 22333
- **What it serves**: The Expo mobile app exported as a web bundle (`expo export --platform web`). This is a standalone, deployable web version of the mobile app — same React code, runs in any browser.

#### Build flow
- `pnpm --filter @workspace/web run build` → invokes `pnpm --filter @workspace/mobile exec expo export --platform web --output-dir ../web/dist/public`
- `expo.experiments.baseUrl` in `artifacts/mobile/app.json` is set to `/web` so all asset paths in the exported bundle are prefixed correctly for the path-based proxy.
- `pnpm --filter @workspace/web run dev` runs `scripts/serve-export.mjs` — a small Express server that serves `dist/public` with SPA fallback. **`dotfiles: "allow"`** is required on `express.static` because Expo emits assets under `assets/__node_modules/.pnpm/...` and the default `.pnpm` dotfile rejection produces silent 404s (font decode errors, blank pages).
- Production deploy uses `serve = "static"` in `artifact.toml` with `publicDir = "artifacts/web/dist/public"` and an SPA rewrite for client-side routing.

#### Key Files
- `scripts/serve-export.mjs` — dev/preview static server (PORT, BASE_PATH from env)
- `dist/public/` — output of `expo export` (gitignored, regenerated by build)
