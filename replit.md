...
### Palava Hub Web (`artifacts/web`)
- **Type**: React + Vite SPA (Facebook-style desktop web experience)
- **Preview path**: `/web/`
- **Port**: 22333
- **What it serves**: A dedicated web-only social experience modeled on Facebook's desktop UI. Connects to the **same Firebase backend** as the mobile app (Auth + Firestore + Storage), so users and posts are fully shared across mobile and web. Mobile app is unchanged.

#### Layout
- Top blue nav bar (logo · search · Home/Friends/Messages/Notifications tabs · avatar menu with logout)
- 3-column desktop layout on `≥xl` screens; collapses to single column on mobile
  - **Left sidebar**: profile shortcut + nav (Friends, Messages, My School, Palava Room, Saved, etc.)
  - **Center**: composer + real-time feed (`onSnapshot` on `posts` collection, ordered by `createdAt desc`, limit 50)
  - **Right sidebar**: trending hashtags (aggregated from recent posts) + contacts list (first 15 users)
- Facebook palette: `--color-fb-blue: #0866FF`, `--color-fb-bg: #f0f2f5`, `--color-fb-card: #fff`

#### Build flow
- `pnpm --filter @workspace/web run dev` → Vite dev server (BASE_PATH=/web/ from artifact env)
- `pnpm --filter @workspace/web run build` → `vite build` with BASE_PATH=/web/ (Replit production preview)
- `pnpm --filter @workspace/web run build:hosting` → `vite build` with BASE_PATH=/ (Firebase Hosting)
- Output: `dist/public/` (gitignored)

#### Key Files
- `src/App.tsx` — router (wouter) with ProtectedLayout / AuthOnly gates
- `src/lib/firebase.ts` — same Firebase config as mobile/admin (project `palava-hub`, `default` Firestore DB)
- `src/context/AuthContext.tsx` — Firebase Auth wrapper (login/register/logout, hydrates Firestore user profile)
- `src/components/TopNav.tsx`, `LeftSidebar.tsx`, `RightSidebar.tsx`, `Avatar.tsx`
- `src/components/PostComposer.tsx` — posts to Firestore + uploads media to Storage (`posts/{userId}/{ts}.ext`), runs `addDoc` + `updateDoc(users, posts: increment(1))` in parallel
- `src/components/PostCard.tsx` — like (optimistic via Firestore transaction on `likedBy[]` + `likes`), comment, share buttons
- `src/pages/Feed.tsx`, `Login.tsx`, `Register.tsx`
