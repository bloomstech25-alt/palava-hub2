...
function StatCard({ label, value, color = "bg-primary", icon }: StatCardProps) {
  return (
    <div className="bg-card border border-card-border rounded-xl p-5 flex items-center gap-4 shadow-sm" data-testid={`card-stat-${label.toLowerCase().replace(/\s+/g, "-")}`}>
...
export default function DashboardPage() {
  const stats = useGetAdminStats({ query: { queryKey: getGetAdminStatsQueryKey() } });
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-1">Platform overview — StudentConnect Liberia</p>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              ...
              icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>}
            />
          </div>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
            ...
            <StatCard
              ...
              color="bg-chart-4"
              icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><path d="M16 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="8.5" cy="7" r="4"/><line x1="20" y1="8" x2="20" y2="14"/><line x1="23" y1="11" x2="17" y2="11"/></svg>}
            />
            <StatCard
              label="New Posts Today"
              value={data.newPostsToday}
              color="bg-chart-1"
              icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>}
            />
            <StatCard
              label="Universities"
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-card border border-card-border rounded-xl p-6">
              <h2 className="text-sm font-semibold text-foreground mb-4">School Breakdown</h2>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-24 h-2 bg-secondary rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: data.totalSchools > 0 ? `${(data.universities / data.totalSchools) * 100}%` : "0%" }}
                      />
                    </div>
                    <span className="text-sm font-medium text-foreground w-6 text-right">{data.universities}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-24 h-2 bg-secondary rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: data.totalSchools > 0 ? `${(data.highSchools / data.totalSchools) * 100}%` : "0%" }}
                      />
                    </div>
                    <span className="text-sm font-medium text-foreground w-6 text-right">{data.highSchools}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-card-border rounded-xl p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-semibold ${data.bannedUsers > 0 ? "text-destructive" : "text-chart-2"}`}>
                    {data.bannedUsers > 0 ? `${data.bannedUsers} banned` : "None"}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Flagged Posts</span>
                  <span className={`text-sm font-semibold ${data.flaggedPosts > 0 ? "text-chart-3" : "text-chart-2"}`}>
